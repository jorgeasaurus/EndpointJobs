  ...
  return (
    ...
    >
      ...
      <div style={{padding: 36}}>
        ...
        <div style={{display: "flex", alignItems: "center", height: 78, marginTop: 32, padding: "0 24px", border: `1px solid ${query ? "rgba(190,242,100,0.46)" : colors.border}`, borderRadius: 14, background: "rgba(0,0,0,0.42)", boxShadow: query ? "0 0 36px rgba(190,242,100,0.08)" : "none"}}>
    ...
    >
      <div>
        ...
        <div style={{display: "flex", gap: 9, marginTop: 17}}>
          {tags.map((tag) => (
            <div key={tag} style={{padding: "7px 10px", borderRadius: 7, background: "rgba(190,242,100,0.075)", color: colors.lime, fontFamily: mono, fontSize: 12, fontWeight: 700}}>{tag}</div>
          ))}
        </div>
      </div>
      <div style={{display: "flex", flexDirection: "column", alignItems: "flex-end", justifyContent: "space-between"}}>
        ...
        <div style={{display: "flex", alignItems: "center", gap: 9, fontFamily: sans, fontSize: 16, fontWeight: 700, color: colors.obsidian, background: colors.lime, padding: "11px 15px", borderRadius: 9}}>View role <ArrowUpRight size={18} strokeWidth={2.5} /></div>
      </div>
    </div>
  );
};
...
export const MiniMap = () => {
  ...
  return (
    <GlassPanel style={{height: 220, position: "relative", background: "rgba(7,10,11,0.92)"}}>
      ...
      <div style={{position: "absolute", left: 25, top: 22, display: "flex", alignItems: "center", gap: 9, color: colors.white, fontFamily: mono, fontSize: 13, fontWeight: 700, letterSpacing: 0.8}}><MapPin size={17} color={colors.emerald} /> JOB GEOGRAPHY</div>
      {points.map((point, index) => {
        const pointIn = snappyReveal(frame, fps, point.delay);
        const pulse = 1 + Math.sin((frame - index * 4) / 8) * 0.07;
        return (
          <div key={`${point.x}-${point.y}`} style={{position: "absolute", left: `${point.x}%`, top: `${point.y}%`, width: 18, height: 18, borderRadius: 20, background: index % 3 === 0 ? colors.lime : colors.emerald, boxShadow: `0 0 0 8px ${index % 3 === 0 ? "rgba(190,242,100,0.11)" : "rgba(45,212,191,0.10)"}, 0 0 24px rgba(190,242,100,0.25)`, opacity: pointIn, transform: `scale(${pointIn * pulse})`}} />
        );
      })}
    </GlassPanel>
  );
...
export const Benefit = ({icon, label}: {icon: "roles" | "salary" | "map"; label: string}) => {
  ...
  return (
    <div style={{display: "flex", alignItems: "center", gap: 13, color: colors.white, fontFamily: sans, fontSize: 21, fontWeight: 600}}>
      <div style={{display: "grid", placeItems: "center", width: 36, height: 36, borderRadius: 10, background: "rgba(190,242,100,0.12)", color: colors.lime}}><Icon size={20} /></div>
      {label}
