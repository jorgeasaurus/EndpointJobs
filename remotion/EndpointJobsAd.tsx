...
const Background = () => {
  ...
  return (
    <AbsoluteFill style={{background: colors.black, overflow: "hidden"}}>
      ...
      <div style={{position: "absolute", inset: -120, transform: `translate(${drift}px, ${drift * 0.35}px)`, opacity: 0.56, backgroundImage: "linear-gradient(rgba(255,255,255,0.042) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.042) 1px, transparent 1px)", backgroundSize: "72px 72px"}} />
      <div style={{position: "absolute", width: 880, height: 880, borderRadius: 1000, left: -330, top: -380, opacity: glow, background: "radial-gradient(circle, rgba(45,212,191,0.18), rgba(45,212,191,0) 68%)"}} />
      <div style={{position: "absolute", width: 980, height: 980, borderRadius: 1000, right: -380, bottom: -520, opacity: glow, background: "radial-gradient(circle, rgba(190,242,100,0.14), rgba(190,242,100,0) 70%)"}} />
    ...
    <SceneShell>
      <div style={{opacity: sceneOpacity}}>
        ...
        <div style={{position: "absolute", top: 255, left: 80}}>
          <div style={{display: "flex", alignItems: "center", gap: 12, color: colors.lime, fontFamily: mono, fontSize: 16, fontWeight: 700, letterSpacing: 1.4, opacity: lineOne}}>
            <Monitor size={21} /> THE JOB BOARD FOR ENDPOINT ENGINEERS
          </div>
          <div style={{fontFamily: sans, color: colors.white, fontSize: 112, fontWeight: 650, letterSpacing: -5.5, lineHeight: 0.96, marginTop: 30}}>
          ...
          <div style={{fontFamily: sans, color: colors.muted, fontSize: 36, fontWeight: 450, letterSpacing: -0.8, marginTop: 36, opacity: subIn, transform: `translateY(${interpolate(subIn, [0, 1], [20, 0])}px)`}}>
    ...
    <AbsoluteFill style={{clipPath: `inset(0 ${100 - sceneIn * 100}% 0 0)`}}>
      ...
      <SceneShell>
        <div style={{position: "absolute", left: 80, top: 245, width: 700, opacity: copyIn, transform: `translateX(${interpolate(copyIn, [0, 1], [-32, 0])}px)`}}>
          <div style={{display: "flex", alignItems: "center", gap: 12, color: colors.emerald, fontFamily: mono, fontSize: 16, fontWeight: 700, letterSpacing: 1.35}}><Search size={21} /> SEARCH WITH SIGNAL</div>
          <div style={{fontFamily: sans, fontSize: 82, fontWeight: 650, lineHeight: 1.02, letterSpacing: -4, color: colors.white, marginTop: 28}}>
        ...
        <div style={{display: "grid", gridTemplateColumns: "1.5fr 0.74fr", gap: 22, marginTop: 62}}>
          ...
          <div style={{display: "flex", flexDirection: "column", gap: 18}}>
            ...
            <GlassPanel style={{height: 245, padding: 28}}>
              <div style={{display: "flex", alignItems: "center", gap: 10, fontFamily: mono, fontSize: 14, fontWeight: 700, letterSpacing: 1, color: colors.muted}}><Command size={18} color={colors.lime} /> YOUR SEARCH, DIALED IN</div>
              ...
              <div style={{height: 8, borderRadius: 20, background: "rgba(255,255,255,0.08)", overflow: "hidden", marginTop: 21}}>
...
const OutroScene = ({siteUrl}: EndpointJobsAdProps) => {
  ...
  return (
    <AbsoluteFill style={{clipPath: `inset(0 0 0 ${100 - sceneIn * 100}%)`}}>
      <AbsoluteFill style={{background: "linear-gradient(145deg, #020a09, #000000)"}} />
      <SceneShell>
      <div style={{position: "absolute", inset: 80, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center"}}>
        <div style={{opacity: brandIn, transform: `translateY(${interpolate(brandIn, [0, 1], [-18, 0])}px)`}}><Brand /></div>
        <div style={{fontFamily: sans, color: colors.white, fontSize: 108, fontWeight: 650, letterSpacing: -5.5, lineHeight: 0.98, marginTop: 65, opacity: titleIn, transform: `translateY(${interpolate(titleIn, [0, 1], [40, 0])}px)`}}>
        ...
        <div style={{position: "relative", display: "flex", alignItems: "center", justifyContent: "space-between", width: 710, height: 96, padding: "0 21px 0 34px", marginTop: 52, borderRadius: 18, border: "1px solid rgba(190,242,100,0.48)", background: "rgba(190,242,100,0.10)", boxShadow: "0 0 70px rgba(190,242,100,0.11)", overflow: "hidden", opacity: ctaIn, transform: `scale(${interpolate(ctaIn, [0, 1], [0.96, 1])})`}}>
          <div style={{fontFamily: mono, color: colors.white, fontSize: 27, fontWeight: 700, letterSpacing: -0.4}}>{siteUrl}</div>
          <div style={{display: "grid", placeItems: "center", width: 58, height: 58, borderRadius: 13, background: colors.lime, color: colors.obsidian}}><ArrowRight size={29} strokeWidth={2.5} /></div>
          <div style={{position: "absolute", left: sweep, top: -80, width: 240, height: 250, transform: "rotate(18deg)", background: "linear-gradient(90deg, transparent, rgba(255,255,255,0.13), transparent)"}} />
        </div>
      </div>
      <div style={{position: "absolute", left: 80, bottom: 70, fontFamily: mono, color: colors.faint, fontSize: 13, fontWeight: 700, letterSpacing: 1.2}}>ENDPOINT · WORKPLACE · CLIENT PLATFORM</div>
      <div style={{position: "absolute", right: 80, bottom: 70, fontFamily: mono, color: colors.faint, fontSize: 13, fontWeight: 700, letterSpacing: 1.2}}>SEARCH WITH SIGNAL</div>
      </SceneShell>
    </AbsoluteFill>
  );
};
